...
const RecommendedContentSchema = z.object({
  ...
  url: z.string().url().describe("The URL of the content. This MUST be a valid placeholder image URL from 'https://picsum.photos'. The format should be 'https://picsum.photos/seed/{a-random-number}/600/400'."),
...
const prompt = ai.definePrompt({
  ...
  Prioritize content from students in the same class, school, or with similar interests. Create realistic but fictional student names for the sources.
  ...
  For each recommendation, provide a compelling title and a reason for the recommendation.

  For the 'url' field, you MUST generate a valid placeholder image URL from 'https://picsum.photos'. The format must be 'https://picsum.photos/seed/{a-random-number}/600/400'. Replace {a-random-number} with a unique random number for each item.
