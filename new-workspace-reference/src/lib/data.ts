...
import type { User, Post, Comment, StudyRoom, Product, FlaggedContent, Game, TeacherClass, GameScore, Tutor, Partner, Transaction, CoinUsageConfig, CoinPackage, GameType, WordScramble, Achievement } from './types';
import { Brain, Puzzle, SpellCheck, Forward, BookA, Medal, Video, Award, MessageSquare, PlusCircle, UserCheck } from 'lucide-react';
...
export const addGame = (newGame: Game) => {
    // Determine the icon based on the game type
    let icon = Puzzle; // Default icon
    if (newGame.gameType === 'Quiz' || newGame.gameType === 'Math Puzzle') icon = BookA;
    if (newGame.gameType === 'Word Scramble') icon = SpellCheck;
    if (newGame.gameType === 'Tongue Twister') icon = Forward;
    if (newGame.gameType === 'Memory Match') icon = Brain;

    games.unshift({...newGame, icon});
};
